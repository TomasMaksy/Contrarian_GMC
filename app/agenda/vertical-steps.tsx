...
const VerticalSteps = React.forwardRef<HTMLButtonElement, VerticalStepsProps>(
	(
		...
		const colors = React.useMemo(() => {
			...
			switch (color) {
				case "primary":
					userColor = "[--step-color:#3fafa8]";
					fgColor = "[--step-color:#3fafa8]";
					break;
				case "secondary":
				...
				case "default":
					...
					break;
				default:
					userColor = "[--step-color:#fff]";
					fgColor = "[--step-color:#fff]";
					break;
			}
						...
						return (
							<li key={stepIdx} className="relative">
								<div className="flex w-full max-w-full items-center gap-2">
									{/* <div className="flex h-full items-center">{step.time}</div> */}
									<button
										key={stepIdx}
										ref={ref}
									...
									>
											...
											<LazyMotion features={domAnimation}>
												<div className="relative">
													<m.div
														animate={status}
														className={cn(
															"relative flex h-[34px] w-[130px] items-center justify-center rounded-xl border-medium text-md font-semibold text-default-foreground",
															{
																"shadow-lg": status === "active",
															}
														)}
														data-status={status}
														initial={false}
										...
										<div className="flex-1 text-left">
											<div>
												<div className="text-white text-xl">{step.title}</div>
								...
								{stepIdx < steps.length - 1 && !hideProgressBars && (
									<div
										aria-hidden="true"
										className={cn(
											"pointer-events-none absolute left-[60] top-[calc(64px_*_var(--idx)_+_1)] flex h-1/2 -translate-y-1/3 items-center px-4"
										)}
										style={{
											// @ts-ignore
											"--idx": stepIdx,
